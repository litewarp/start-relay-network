...
middleware, and response transforms — the function returns the appropriate
...
export const getRelayEnvironment = createIsomorphicFn()
  .client(() => createRelayEnvironment({ url, isServer: false }))
  .server(() => createRelayEnvironment({ url, isServer: true }));
```
